...
- Job ID: `job-20260916-013209-pi-ce7f0a2d`
- Title: Pi 코딩 에이전트 설치
- Goal: 공식 pi.dev 배포판을 Windows에 설치하고 실행 가능 상태를 검증한다
- Status: `PENDING`
...
- Task ID: `task-20260916-013216-pi-cli-576f5881`
- Title: Pi CLI 설치 및 검증
- Description: pi.dev 공식 Pi 코딩 에이전트를 현재 Windows 환경에 전역 설치한다.
- Expected output: pi 명령을 어느 터미널에서든 실행할 수 있다.
- Done condition: pi --version이 정상 종료되고 설치된 버전을 출력한다.
- Validation hint: node --version, npm --version, pi --version을 실행한다.
- Required docs: -
- Memory refs: -
- Document outputs: -
...
- In scope: Node/npm 사전 조건 확인, 공식 npm 패키지 전역 설치, 버전 검증
- Out of scope: 모델 제공자 로그인 및 API 키 등록, 서드파티 확장 설치
